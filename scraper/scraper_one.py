...
class BBCNewsScraper:
    ...
    def fetch_feed_content(self, url, max_retries=None, timeout=None):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning(f"[BBC Scraper] Attempt {attempt}/{max_retries} failed for {url}: {e}")
                if attempt < max_retries:
                    time.sleep(1 * attempt)
        logger.error(f"[BBC Scraper] Failed to fetch {url} after {max_retries} attempts.")
        return None
    ...
    def parse_published_date(self, parsed_entry):
        ...
        if hasattr(parsed_entry, 'published_parsed') and parsed_entry.published_parsed:
            try:
                ...
                logger.debug(f"[BBC Scraper] Date parsing error from published_parsed: {e}")
    ...
    def extract_image_url(self, entry, summary_html=""):
        """Extract image URL from feed enclosures, media content, or HTML summary."""
        # Check feed enclosures/media
        ...
        # Fallback to parsing HTML summary/content using BeautifulSoup
        if summary_html:
        ...
        # Fallback to unique seed image based on article URL hash
        import hashlib
        url_hash = hashlib.md5((entry.get('link', '') or entry.get('title', '')).encode('utf-8')).hexdigest()
        ...
        for entry in parsed_feed.entries:
            try:
                ...
                articles.append({
                    ...
                    'summary': summary_text[:500],  # Truncate summary if long
                    'image_url': image_url,
                    'article_url': article_url,
                    'country': 'UK'
                })
            except Exception as e:
                logger.error(f"[BBC Scraper] Error processing entry '{getattr(entry, 'title', 'Unknown')}': {e}")
                continue
