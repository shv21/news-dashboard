...
class AustraliaNewsScraper:
    ...
    def fetch_feed_content(self, url, max_retries=None, timeout=None):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning(f"[ABC Australia Scraper] Attempt {attempt}/{max_retries} failed: {e}")
    ...
    def scrape(self):
        ...
        logger.info(f"[ABC Australia Scraper] Starting scrape from {self.RSS_URL}")
        feed_xml = self.fetch_feed_content(self.RSS_URL)
        if not feed_xml:
            logger.error("[ABC Australia Scraper] Unable to fetch ABC RSS feed xml.")
            return []
        ...
        logger.info(f"[ABC Australia Scraper] Successfully extracted {len(articles)} articles.")
        return articles
