...
class CBCNewsScraper:
    ...
    RSS_URL = "https://globalnews.ca/feed/"
    ...
    def fetch_feed_content(self, url, max_retries=None, timeout=None):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning(f"[Canada Scraper] Attempt {attempt}/{max_retries} failed: {e}")
    ...
    def scrape(self):
        """Execute news scraping for Canadian News."""
        logger.info(f"[Canada Scraper] Starting scrape from {self.RSS_URL}")
        feed_xml = self.fetch_feed_content(self.RSS_URL)
        if not feed_xml:
            logger.error("[Canada Scraper] Unable to fetch Canada RSS feed xml.")
            return []
        ...
        for entry in parsed_feed.entries:
            try:
                ...
                logger.error(f"[Canada Scraper] Error parsing entry: {e}")
                continue

        logger.info(f"[Canada Scraper] Successfully extracted {len(articles)} articles.")
        return articles
